...
/* ===========================
   ANNOUNCEMENT BAR (1 → 4 novembre)
=========================== */
function AnnouncementBar() {
  const KEY = "bannerDismissed-2025-11-01";
  ...
  const href =
    LINKS.anunnaki1Hardcover && LINKS.anunnaki1Hardcover.trim().length > 0
      ? LINKS.anunnaki1Hardcover
      : "#libri";

  return (
    <div className="w-full bg-gradient-to-r from-[#0e1b44] via-[#14214e] to-[#0e1b44] border-b border-white/10">
      <div className="mx-auto max-w-7xl px-4 py-2 md:py-3 flex flex-col md:flex-row items-center justify-between gap-3">
        <p className="text-sm md:text-base text-white/90 text-center md:text-left">
          <span className="text-gold font-semibold">Novità</span> —{" "}
          <span className="font-medium">“Codice Anunnaki – Gli Dèi del Cielo e della Terra”</span>:{" "}
          copertina rigida <span className="text-gold font-semibold">in anteprima su Amazon dall’1 novembre</span>.{" "}
          Tutte le altre versioni al lancio ufficiale{" "}
          <span className="text-gold font-semibold">4 novembre</span>.
        </p>
        ...
        <div className="flex items-center gap-3">
          <a
            ...
            className="btn-gold rounded-xl px-4 py-2 text-sm md:text-base"
          >
            Vai all’anteprima
          </a>
          <button
            ...
            className="inline-flex items-center justify-center w-9 h-9 rounded-lg bg-white/5 border border-white/10 text-white/70 hover:text-gold hover:border-gold/60"
          >
            ✕
          </button>
        </div>
      ...
      <header className="sticky top-0 z-40 backdrop-blur bg-[#0a1230]/70 border-b border-white/10">
        <div className="mx-auto max-w-7xl px-4 py-3 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-4">
            <SocialBar />
            <a href="#libri" className="btn-gold rounded-xl">
              Acquista i Libri
            </a>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-7xl px-4 py-20 md:py-28 relative">
          <div className="max-w-3xl">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              <a href="#contatti" className="btn-outline rounded-xl">
                Contattami
              </a>
            </div>
          </div>
    ...
    const obs = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          setInView(true);
          setTimeout(() => setInView(false), 1000); // ri-triggerabile scorrendo
        }
      },
